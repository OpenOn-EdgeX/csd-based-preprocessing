...
#
# 사용법:
#   ./server/copy_data.sh          (기본: 10장씩 배치 전송)
#   ./server/copy_data.sh --all    (전체 한번에 전송)
##############################################################################

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_DIR="$(dirname "$SCRIPT_DIR")"
SOURCE_DIR="$PROJECT_DIR/demo_data/_source/images"
...
echo ""
echo -e "${GREEN}============================================${NC}"
echo -e "${GREEN}  Edge Device -> CSD Data Transfer (cp)${NC}"
echo -e "${GREEN}============================================${NC}"
echo ""

mapfile -t FILES < <(find "$SOURCE_DIR" -maxdepth 1 -type f -name '*.jpg' | sort)
TOTAL=${#FILES[@]}
...
if [ "$1" == "--all" ]; then
    echo -e "${YELLOW}[Edge]${NC} Copy $TOTAL images at once..."
    if ! "${SCP_CMD[@]}" "${FILES[@]}" "$CSD_HOST:$CSD_TARGET/"; then
    ...
    while [ $SENT -lt $TOTAL ]; do
        ...
        echo -e "${YELLOW}[Edge]${NC} Copy batch: ${BATCH_COUNT} images (${END}/${TOTAL})..."

        BATCH_FILES=()
        for ((i=SENT; i<END; i++)); do
            BATCH_FILES+=("${FILES[$i]}")
        done
        if ! "${SCP_CMD[@]}" "${BATCH_FILES[@]}" "$CSD_HOST:$CSD_TARGET/"; then
            echo "Error: batch transfer failed at ${END}/${TOTAL}"
            exit 1
        fi
